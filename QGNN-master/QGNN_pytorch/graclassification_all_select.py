...
parser.add_argument("--num_GNN_layers", default=1, type=int, help="Number of hidden layers")
...
model = SupQGNN(feature_dim_size=feature_dim_size*4,  # A + Ai + Aj + Ak
                ...
                num_classes=num_classes,freeze_epoch=freeze_epoch).to(device)
...
def train():
    ...
    for _ in range(num_batches_per_epoch):
        ...
        optimizer.zero_grad()
        prediction_scores = model(Adj_block, X_concat, graph_pool,epoch)
...
def evaluate():
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(test_graphs), args.batch_size):
            ...
            prediction_scores = model(Adj_block, X_concat, graph_pool,now_epoch=1000).detach()
