...
parser.add_argument("--num_GNN_layers",  type=int, help="Number of hidden layers")
parser.add_argument("--hidden_size", type=int)
parser.add_argument("--model",  help="GatedQGNN, GatedGNN")
...
model = QGNN(nfeat=feature_dim_size, nhid=args.hidden_size, nclass=num_classes, dropout=args.dropout,freeze_epoch=freeze_epoch).to(device)
...
def train():
    ...
    for start in range(0, len(train_y), args.batch_size):
        ...
        prediction_scores = model(torch.from_numpy(train_feature[idx]).float().to(device),
                                  ...
                                  torch.from_numpy(train_mask[idx]).float().to(device),epoch)
...
def evaluate(tmp_feature, tmp_adj, tmp_mask, tmp_y):
    ...
    with torch.no_grad():
        prediction_scores = model(torch.from_numpy(tmp_feature).float().to(device),
                                  ...
                                  torch.from_numpy(tmp_mask).float().to(device),epoch).detach()
