...
class QGNN(torch.nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout=0.5,freeze_epoch=200):
        ...
        self.menk1= QGNNLayer(nhid//3, nhid//3, dropout=dropout, quaternion_ff=False,act=lambda x:x)
        self.menk2= QGNNLayer(nhid//3, nhid//3, dropout=dropout, quaternion_ff=False,act=lambda x:x)
        self.menk3= QGNNLayer(nhid//3, nhid//3, dropout=dropout, quaternion_ff=False,act=lambda x:x)


        self.hid=nhid
        self.q4gnn0 = QGNNLayer(nfeat , nhid, dropout=dropout)
        self.q4gnn1 = QGNNLayer(nfeat , nhid, dropout=dropout) # prediction layer
        self.q4gnn2 = ALL_select(nhid//3, nhid//3, dropout=dropout,freeze_epoch=freeze_epoch)
        self.q4gnn3 = ALL_select(nhid//3, nhid//3, dropout=dropout,freeze_epoch=freeze_epoch)
        self.q4gnn4 = ALL_select(nhid//3, nhid//3, dropout=dropout,freeze_epoch=freeze_epoch)
        self.q4gnn5 = ALL_select(nhid//3, nhid//3, dropout=dropout,freeze_epoch=freeze_epoch)
        self.out = QGNNLayer(nhid, nhid, dropout=dropout, quaternion_ff=False, act=lambda x:x) # prediction layer
        self.graph_channel=Graph_channel(nhid)
        self.prediction = nn.Linear(nhid, nclass)
        self.dropout = nn.Dropout(dropout)
        # self.out=QGNNLayer(nclass, nclass, dropout=dropout, quaternion_ff=False, act=lambda x:x) # prediction layer
    def forward(self, x, adj,mask,now_epoch):

        y0=self.q4gnn0(x,adj)
        # y0=self.fnet(y0)
        x = self.q4gnn1(x, adj)
        x1,x2,x3=torch.split(x,self.hid//3,dim=2)
